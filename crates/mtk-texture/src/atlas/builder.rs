...
impl AtlasBuilder {
    ...
    pub fn build_from_sprites_with_category(
        ...
        // =========================================================================
        // 2. Build Dedicated Animated Atlas Chunks (Full Strips + PBR Tiling)
        // =========================================================================
            ...
            for (idx, chunk) in anim_stitched.chunks.into_iter().enumerate() {
                ...
                for slot in chunk.slots {
                    if let Some(sp) = anim_sprite_map.get(&slot.entry) {
                        ...
                        let fw = sp.frame_width;
                        let fh = sp.frame_height;
                        let strip_w = sp.albedo.width;
                        let strip_h = sp.albedo.height;

                        // 1. Blit full Albedo animation strip
                        albedo_buf.blit(&sp.albedo, 0, 0, inner_x, inner_y, strip_w, strip_h);
                        if self.config.padding > 0 {
                            apply_edge_clamping_padding(&mut albedo_buf, inner_x, inner_y, strip_w, strip_h, self.config.padding);
                        }

                        // 2. Blit Normal companion with auto-tiling if frame count < albedo frames
                        let slot_has_normal = sp.normal.is_some();
                        if let (Some(ref mut n_buf), Some(ref norm_src)) = (&mut normal_buf, &sp.normal) {
                            let aligned_normal = if norm_src.height < strip_h {
                                norm_src.tile_vertical(strip_h)
                            } else {
                                norm_src.clone()
                            };
                            n_buf.blit(&aligned_normal, 0, 0, inner_x, inner_y, strip_w, strip_h);
                            if self.config.padding > 0 {
                                apply_edge_clamping_padding(n_buf, inner_x, inner_y, strip_w, strip_h, self.config.padding);
                            }
                        }

                        // 3. Blit Specular companion with auto-tiling if frame count < albedo frames
                        let slot_has_specular = sp.specular.is_some();
                        if let (Some(ref mut s_buf), Some(ref spec_src)) = (&mut specular_buf, &sp.specular) {
                            let aligned_spec = if spec_src.height < strip_h {
                                spec_src.tile_vertical(strip_h)
                            } else {
                                spec_src.clone()
                            };
                            s_buf.blit(&aligned_spec, 0, 0, inner_x, inner_y, strip_w, strip_h);
                            if self.config.padding > 0 {
                                apply_edge_clamping_padding(s_buf, inner_x, inner_y, strip_w, strip_h, self.config.padding);
                            }
                        }
